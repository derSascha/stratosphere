...
public class ProfilingSubgraph {
	...
	public ProfilingSubgraph(ExecutionGraph executionGraph, ExecutionGroupVertex subgraphStart,
		...
		for (EdgeCharacteristics edgeChar : edgeCharacteristics.values()) {
			ManagementEdge edge = edgeChar.getEdge();
			String humanReadable = ProfilingUtils.formatName(edge);
			String ids = edge.getSource().getVertex().getID().toString() + "->" + edge.getTarget().getVertex().getID().toString();
