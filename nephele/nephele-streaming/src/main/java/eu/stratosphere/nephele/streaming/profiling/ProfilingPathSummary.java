			...
			for (ManagementAttachment element : pathElements) {
				...
				if (element instanceof ManagementVertex) {
					...
					if (vertexLatency.getLatencyInMillis() == -1) {
						LOG.info("no data for vertex " + ProfilingUtils.formatName((ManagementVertex) element));					
						this.hasLatencies = false;
						break;
					}
				} else {
					...
					if (edgeChar.getChannelLatencyInMillis() == -1 ||
						edgeChar.getOutputBufferLifetimeInMillis() == -1) {
						LOG.info("no data for edge " + ProfilingUtils.formatName((ManagementEdge) element));
						this.hasLatencies = false;
						break;
					}
				}
	...
	public void refreshLatencies() {
		...
		for (ManagementAttachment element : pathElements) {
			...
			if (element instanceof ManagementVertex) {
				...
				latencies[index] = edgeCharacteristics.getOutputBufferLifetimeInMillis() / 2;
