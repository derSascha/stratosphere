...
/**
 * This class provides an abstract base class for an input gate wrapper. An
 * input gate wrapper can be used by a plugin to wrap an input gate and
 * intercept particular method calls. The default implementation of this
 * abstract base class simply forwards every method call to the encapsulated
 * input gate.
 * <p>
 ...
 */
public abstract class AbstractInputGateWrapper<T extends Record> implements
		InputGate<T> {
	...
	public AbstractInputGateWrapper(final InputGate<T> wrappedInputGate) {
		...
		if (wrappedInputGate == null) {
			throw new IllegalArgumentException(
					"Argument wrappedInputGate must not be null");
		}
	 ...
	 */
	@Override
	public void subscribeToEvent(final EventListener eventListener,
	 ...
	 */
	@Override
	public void publishEvent(final AbstractTaskEvent event) throws IOException,
	...
	public NetworkInputChannel<T> createNetworkInputChannel(
		...
		return this.wrappedInputGate.createNetworkInputChannel(inputGate,
